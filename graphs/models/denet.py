...
class Denet(nn.Module):
    def __init__(self, config):
        ...
        self.att_dim_out         = int(self.sinc_dim_out*([(1/3) - (5/(3*self.sinc_dim_out)),1][not self.config.DenetBeforePooling]))
        self.attention_layer    = DELayer(self.sinc_dim_out,input_channels = 80,sum_channels=self.config.SumChannel, dropout = config.DenetDropout) 
    ...
    def get_conv(self,input_dim,input_channels,out_channels=60):
        return nn.Sequential(
            nn.Conv1d(input_channels,out_channels,kernel_size=5),nn.MaxPool1d(3),nn.LayerNorm([out_channels,((input_dim-5)//3)+1]),nn.LeakyReLU()
        )
    ...
    def forward(self, x):
        ...
        if self.config.DenetBeforePooling : x = self.attention_layer(x)
        
        x = nn.MaxPool1d(kernel_size=3)(x)
        
        x = nn.LeakyReLU()(x)
        if not self.config.DenetBeforePooling : x = self.attention_layer(x)
        ...
        s1 = x.shape
        if self.config.DenetDropout != 0 : x = self.dropout(x)
        s2 = x.shape
        x = self.conv2(x)
        s3 = x.shape
        if self.config.DenetDropout != 0 : x = self.dropout(x)
